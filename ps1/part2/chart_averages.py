...
def make_chart(points):
    info = sorted(points, key=lambda x: int(x.dbm))
    # data = np.array([(b.dbm, b.avg) for b in info])
    data = []
    for b in info:
        data.append((b.dbm, b.avg))
    ...
    labels = [b.dbm for b in info]
    plt.scatter(*zip(*data))
    plt.axis([-100, -40, 0, 300])
    plt.savefig("averages.png")


def main():
    points = read_lines()
    chart = make_chart(points)
